...
function applyTheme(hue: number, mode: 'light' | 'dark' | 'white') {
  ...
  if (mode === 'light' && !neutral) {
    ...
    set('--accent-50',       `${h} 30% 97%`)
    set('--accent-100',      `${h} 32% 90%`)
    set('--accent-200',      `${h} 36% 78%`)
    set('--accent-300',      `${h} 40% 63%`)
    set('--accent-400',      `${h} 42% 50%`)
    set('--accent-500',      `${h} 44% 40%`)
    set('--accent-600',      `${h} 45% 32%`)
    set('--accent-700',      `${h} 44% 24%`)
    set('--accent-800',      `${h} 40% 17%`)
    set('--accent-900',      `${h} 36% 11%`)
  } else if (mode === 'dark') {
    // White + Dark = neutral dark grey
    set('--bg-primary',      '0 0% 8%')
    set('--bg-secondary',    '0 0% 12%')
    set('--bg-tertiary',     '0 0% 16%')
    set('--bg-calendar',     '0 0% 20%')
    set('--surface',         '0 0% 14%')
    set('--surface-hover',   '0 0% 19%')
    set('--surface-active',  '0 0% 23%')
    set('--border',          '0 0% 27%')
    set('--border-subtle',   '0 0% 19%')
    set('--text-primary',    '0 0% 92%')
    set('--text-secondary',  '0 0% 62%')
    set('--text-muted',      '0 0% 42%')
    set('--accent-50',       '0 0% 97%')
    set('--accent-100',      '0 0% 90%')
    set('--accent-200',      '0 0% 80%')
    set('--accent-300',      '0 0% 68%')
    set('--accent-400',      '0 0% 62%')
    set('--accent-500',      '0 0% 58%')
    set('--accent-600',      '0 0% 58%')
    set('--accent-700',      '0 0% 48%')
    set('--accent-800',      '0 0% 34%')
    set('--accent-900',      '0 0% 23%')
  } else {
    // White + Light (or mode==='white') = pure white
